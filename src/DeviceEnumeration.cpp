			...
			if (SUCCEEDED(hr))
			{
				...
				for (dwIndex = 0; dwIndex < *cPnPDeviceIDs; dwIndex++)
				{
					...
					if ((hr == S_OK) && (cchFriendlyName > 0))
					{
						...
						if (pszFriendlyName != NULL)
						{
							hr = pPortableDeviceManager->GetDeviceFriendlyName(pPnpDeviceIDs[dwIndex], pszFriendlyName, &cchFriendlyName);
							if (SUCCEEDED(hr))
							{
								//printf("Friendly Name: %ws\n", friendlyName);
								//printf("Friendly Name: %ws\n", pszFriendlyName);
								if (wcscmp(friendlyName, pszFriendlyName) == 0)
								{
									result = dwIndex;
									break;
								}
